...
public class ProductTypePanel extends JPanel implements ModifyPanel {
   ...
   public ProductTypePanel(ProductType productType, ModifyPanelListener listener) throws Exception {
   ...
   private void setValues() throws Exception {
      this.isNew = !this.productType.exists((TransactionContainer)null);
      if(this.isNew) {
         this.lTitle.setText("New ProductType");
         this.tId.setText("");
         this.tId.setEnabled(true);
         this.tDisplayOrder.setText("");
         this.tName.setText("");
      } else {
         this.lTitle.setText("Editing " + this.productType.toString());
         this.tId.setText(Integer.toString(this.productType.getObjectId()));
         this.tId.setEnabled(false);
         this.tDisplayOrder.setText(Integer.toString(this.productType.getDisplayOrder()));
         this.tName.setText(this.productType.getName());
      }

      this.tLastUpdate.setText(ViewUtil.calendarDisplay(this.productType.getLastUpdate()));
      this.tNotes.setText(this.productType.getNotes());
   }
   ...
   public void save() throws Exception {
      ...
      if(this.isNew) {
         if(id.length() == 0) {
            this.productType.setObjectId(Settings.getNullInt());
         } else {
            try {
               this.productType.setObjectId(Integer.parseInt(id));
            } catch (Exception var4) {
               throw new UserInputException("Invalid ProductType Id format: integer required", this.tId);
            }

            if(this.productType.exists((TransactionContainer)null)) {
               throw new UserInputException("Invalid ProductType Id: this Id is already in use", this.tId);
            }
         }
      }

      this.productType.setName(this.tName.getText().trim());

      try {
         this.productType.setDisplayOrder(Integer.parseInt(this.tDisplayOrder.getText()));
      } catch (Exception var3) {
         throw new UserInputException("Invalid Display Order format: integer required", this.tDisplayOrder);
      }

      this.productType.setNotes(this.tNotes.getText().trim());
      this.productType.save((TransactionContainer)null);
      this.productType.setLastUpdate(Calendar.getInstance());
      this.listener.saveCompleted(this.productType, this.isNew);
      if(this.isNew) {
         this.setValues();
      }
